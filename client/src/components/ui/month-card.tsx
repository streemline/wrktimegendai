...
export function MonthCard({ report, className }: MonthCardProps) {
  const progressPercentage = report.targetMinutes > 0 
  ...
  return (
    ...
    >
      <Card className={cn("bg-card mb-4", className, isHovered ? "shadow-lg" : "")}>
        <CardContent className="p-4">
          ...
          >
            <Progress className="h-2 mt-4 bg-gray-700" value={progressPercentage} />
          </motion.div>
          ...
          <div className="flex justify-between text-sm mt-2">
            <motion.span
              ...
              transition={{ duration: 0.2 }}
            >
              {formatMinutesToHours(report.workedMinutes)} ч / {formatMinutesToHours(Math.abs(report.targetMinutes))} ч
            </motion.span>
            <motion.span
