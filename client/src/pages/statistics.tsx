...
import {
  ...
  AnimatedEntryRow,
  AnimatedFade,
} from "@/components/ui/animated-entry";
import {
  AnimatedActionButton,
  AnimatedActionGroup,
  AnimationType,
} from "@/components/ui/animated-action-button";
import {
  MoodRatingDisplay,
  EnergyLevelDisplay,
} from "@/components/ui/mood-energy-selector";
...
import ProductivityRewards from "@/components/gamification/productivity-rewards";
...
export default function Statistics() {
  ...
  const { data: currentMonthReport, isLoading: isReportLoading } =
  ...
  const deleteEntryMutation = useMutation({
    mutationFn: (entryId: number) =>
      apiRequest("DELETE", `/api/time-entries/${entryId}`),
    onSuccess: () => {
      toast({
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (entriesForDay.length > 0) {
        ...
        rows.push({
          ...
          animationType: entry.animationType || "spring",
        });
      } else {
        rows.push({
          day,
  ...
  const sortedReports =
    allMonthlyReports && Array.isArray(allMonthlyReports)
      ? [...allMonthlyReports]
          .sort((a, b) => {
  ...
  // Обработчик изменения диапазона дат для аналитики
  const handleDateRangeChange = (startDate: Date, endDate: Date) => {
    // В будущем можно добавить логику запроса данных за выбранный диапазон
        ...
        <div className="bg-card border-b border-gray-700">
          <div className="container mx-auto">
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pt-2">
                <TabsList className="mb-2 sm:mb-0">
                  <TabsTrigger value="timesheet" className="gap-2">
                    ...
                    <span className="hidden sm:inline">Час</span>
                  </TabsTrigger>
                          ...
                          <div className="flex flex-col space-y-4">
                              ...
                              <div className="flex items-center">
                                ...
                                <span className="ml-auto font-medium">
                                  {formatMinutesToHours(
                                    currentMonthReport.workedMinutes,
                                  )}{" "}
                                  ч
                                </span>
                              </div>
                            ...
                            <AnimatedEntry delay={0.3}>
                              <div className="flex items-center mt-3">
                                <div className="text-primary mr-3 h-5 w-5">
                                  ₴
                                </div>
                            ...
                            <AnimatedEntry delay={0.4}>
                              <div className="mt-2 flex justify-between items-center">
                                <div className="text-sm text-muted-foreground">
                                  <span>
                                    Робочі дні: {currentMonthReport.workDays} д
                                  </span>
                                </div>
                                <div className="text-sm text-muted-foreground">
                                  <span>
                                    Робочі години:{" "}
                                    {formatMinutesToHours(
                                      currentMonthReport.targetMinutes,
                                    )}{" "}
                                    ч
                                  </span>
                                </div>
                    ...
                    <table className="w-full bg-card rounded-lg shadow-lg">
                      <thead>
                        <tr className="text-muted-foreground border-b border-gray-700">
                          ...
                          <th className="p-2 text-center">Настрій</th>
                            ...
                            >
                              ...
                              <td className="p-2 text-center">
                                <div className="flex flex-col items-center justify-center">
                                  <MoodRatingDisplay rating={row.moodRating} />
                                  {row.energyLevel && (
                                    <motion.div
                                ...
                                {row.entryId ? (
                                  <AnimatedActionGroup>
                                    <AnimatedActionButton
                                      ...
                                      onClick={() =>
                                        handleEditEntry(row.entryId!)
                                      }
                                      hoverColor="text-primary"
                                      label="Редагувати"
                                      showTooltip={false}
                                      animationType={
                                        (row.animationType as AnimationType) ||
                                        "spring"
                                      }
                                    />
                                    <AnimatedActionButton
                ...
                >
                  <AlertDialogContent className="bg-card">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Это действие удалит запись о рабочем времени. Это
              ...
              {/* Аналитика */}
              <TabsContent value="analytics" className="mt-0 mx-4">
                {!isReportLoading &&
                  !isEntriesLoading &&
                  !isAllReportsLoading &&
                  timeEntries &&
              ...
              {/* Достижения */}
              <TabsContent value="achievements" className="mt-0 mx-4">
                {!isReportLoading &&
                  !isEntriesLoading &&
                  !isAllReportsLoading &&
                  user &&
