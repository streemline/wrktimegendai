...
export async function generatePdfExport(
  ...
  try {
    ...
    const month = dates.getMonthName(monthlyReport.month);
    ...
    entries.forEach((entry, index) => {
      ...
      for (let i = 0; i < rowData.length; i++) {
        ...
        let text = rowData[i];
        if (i === 1 && text.length > 20) {
          // Для колонки с заметками
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold");
    doc.text("soucet hodin:", 15, yPos);
    yPos += 7;

    doc.setFont("helvetica", "normal");
    doc.text(
      `CELKEM K UHRADE: ${time.formatMinutesToHours(monthlyReport.workedMinutes)} hodin`,
      15,
      yPos,
    );
    yPos += 7;

    doc.text(`ZBYVA K UHRADE: ${totalPayment} CZK`, 15, yPos);
    yPos += 7;

    // Подпись
    doc.text("podpis pracovnika:", 100, yPos);
...
export async function generateExcelExport(
  ...
  const titleRows = [
    ["Výkaz práce"],
    [`${dates.getMonthName(monthlyReport.month)} ${monthlyReport.year}`],
    [],
  ];
  ...
  const wsData = [
    ...
    [
      "Celkem odpracováno:",
      `${time.formatMinutesToHours(monthlyReport.workedMinutes)} hodin`,
    ],
...
export async function generateCsvExport(
  ...
  // Add headers
  csvContent +=
    "Дата,День тижня,Початок,Кінець,Перерва,Відпрацьовано,Примітки\n";
  ...
  // Add summary
  csvContent += "\nЗведення\n";
  csvContent += `Місяць,${dates.getMonthName(monthlyReport.month)} ${monthlyReport.year}\n`;
  csvContent += `Робочі дні,${monthlyReport.workDays}\n`;
  csvContent += `Відпрацьовано,${time.formatMinutesToHours(monthlyReport.workedMinutes)}\n`;
  csvContent += `Понаднормові,${time.formatMinutesToHours(monthlyReport.overtimeMinutes)}\n`;
